...
function SimpleApp() {
  ...
  // 历史项 hover tooltip：APK 文件名置顶，下面列安装记录（设备 SN · 次数 · 时间，多行）。换行靠 \n（GlobalTooltip white-space:pre-line）。
  const buildApkHistoryTip = (h: ApkHistoryItem): string => {
    if (h.devices.length === 0) return `${h.fileName}\n安装记录：装过 ${h.installCount} 次（早期记录无设备信息）`;
    const lines: string[] = [h.fileName, '安装记录：'];
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', minHeight: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: '12px' }}>
                  <span
                    className="link"
                    onClick={(e) => { e.stopPropagation(); clearPendingApks(); }}
                    style={{ fontSize: '12px', cursor: isUnifiedInstalling ? 'not-allowed' : 'pointer', opacity: isUnifiedInstalling ? 0.5 : 1, color: 'var(--fg-tertiary)' }}
                  >
                    {'清空已选'}
                  </span>
